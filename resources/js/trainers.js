var deleteFunction = function (id) {
    let r = confirm('Do you really want to delete this trainer?');
    if (r === true) {
        window.location.href = 'http://127.0.0.1:5500/deletetrainer?id=' + id;
    }
}

$(document).ready(function () {
    var JSONPath = "https://api.myjson.com/bins/vb60z";
    ...
    $('#Modal').on('show.bs.modal', function (event) {
        ...
        if(message == "Edit Trainer") {
            $('#modalForm').attr('action', '/updatetrainer' );
        } else {
            $('#modalForm').attr('action', '/addtrainer' );
        }
      })
